...
- (void)viewDidLoad
{
    ...
    // 1: Allocate a context for us to draw in using OPENGLES2
    ...
    // 2: Set our content scale factor for when full screen is enabled
    ...
    // 3: Set our perferred GPU frame rate
    nGPUPreferredFPS = 60;
...
- (void)update
{
    //miOSNativeApp->Update();
    // Ok lets set that we are
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    // iOS had a bad habit of sending in dupicates and race conditions when the user is hiting
